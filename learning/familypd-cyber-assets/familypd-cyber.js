const FPD = (() => {
  ...
  function initTerminal(){const input=$('#termIn'),output=$('#termOut'),run=$('#runCmd');if(!input||!output||!run)return;const outputs={'pwd':'/home/victimpi','ls':'Documents  Downloads  linux-lab  notes.txt','ls -la':'drwxr-xr-x  5 victimpi victimpi 4096 .\ndrwxr-xr-x  3 root     root     4096 ..\ndrwxr-xr-x  2 victimpi victimpi 4096 Documents\ndrwxr-xr-x  2 victimpi victimpi 4096 Downloads\ndrwxr-xr-x  3 victimpi victimpi 4096 linux-lab\n-rw-r--r--  1 victimpi victimpi   77 notes.txt','lsblk':'NAME         SIZE FSTYPE MOUNTPOINTS\nmmcblk0       32G\n├─mmcblk0p1  512M vfat   /boot/firmware\n└─mmcblk0p2 31.5G ext4   /\nsda           16G\n└─sda1        16G exfat  /media/victimpi/CLASS_USB','df -h':'Filesystem       Size  Used Avail Use% Mounted on\n/dev/mmcblk0p2   31G   11G   19G  37% /\n/dev/sda1        16G  4.2G   12G  27% /media/victimpi/CLASS_USB','mount':'/dev/mmcblk0p2 on / type ext4 (rw,relatime)\n/dev/sda1 on /media/victimpi/CLASS_USB type exfat (rw,nosuid,nodev)','cat notes.txt':'Read-only exploration: identify the USB device, its file system, mount point, and available space.','clear':'__CLEAR__'};function exec(cmd){cmd=cmd.trim();if(!cmd)return;const result=outputs[cmd]||'That command is not included in this safe simulator. Try: pwd, ls, ls -la, lsblk, df -h, mount, cat notes.txt, or clear.';if(result==='__CLEAR__')output.textContent='';else output.textContent+=`\nvictimpi@lab:~$ ${cmd}\n${result}\n`;output.scrollTop=output.scrollHeight;input.value='';}run.addEventListener('click',()=>exec(input.value));input.addEventListener('keydown',e=>{if(e.key==='Enter')exec(input.value)});$$('[data-cmd]').forEach(btn=>btn.addEventListener('click',()=>exec(btn.dataset.cmd)))}
  ...
  return { init(){initRevealCards();initSingleChoiceQuiz();initResetButtons();initComponentCards();initPorts();initConverter();initGlossary();initTerminal();initFacts();initVersionQuestions();initCredentialDemo();} };
})();
